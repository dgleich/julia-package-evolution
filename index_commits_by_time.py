#!/usr/bin/env python3
"""
index_commits_by_time.py

This script indexes commits in the Julia Registry by time, selecting the first commit
for each day. The result is stored in a structured format for later analysis.
"""
...
def index_commits_by_hour(commits):
    """Group commits by hour and select the first commit of each hour."""
    hours = defaultdict(list)
    
    # Group commits by hour
    for commit in commits:
        hour_key = commit["date"].strftime("%Y-%m-%d %H")
        hours[hour_key].append(commit)
    
    # Sort commits within each hour and select the first one
    first_commits_by_hour = {}
    for hour, hour_commits in hours.items():
        # Sort by datetime (earliest first)
        hour_commits.sort(key=lambda x: x["date"])
        first_commit = hour_commits[0]
        
        first_commits_by_hour[hour] = {
            "hash": first_commit["hash"],
            "datetime": first_commit["date"].isoformat()
        }
    
    return first_commits_by_hour
...
def main():
    parser = argparse.ArgumentParser(description="Index commits in the Julia Registry by time")
    parser.add_argument(
        "--granularity", 
        choices=["day", "month", "hour"], 
        default="day",
    ...
    parser.add_argument(
        "--output", 
        default=None,
        help="Output file path (default: commits_by_<granularity>.json)"
    )
    
    args = parser.parse_args()
    
    if args.output is None:
        args.output = f"commits_by_{args.granularity}.json"
    
    # Get all commits
    print(f"Fetching all commits from repository...")
    ...
    elif args.granularity == "hour":
        index = index_commits_by_hour(all_commits)
